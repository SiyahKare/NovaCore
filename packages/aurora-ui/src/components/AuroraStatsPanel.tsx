...
export const AuroraStatsPanel: React.FC<AuroraStatsPanelProps> = ({
  ...
  const loadStats = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiUrl}/admin/aurora/stats`, {
        headers: {
  ...
  if (loading && !stats) {
    return (
      <div className={`p-6 text-sm text-slate-400 ${className}`}>
        Aurora istatistikleri yükleniyor...
      </div>
    )
  }
  ...
  return (
    <div className={`p-6 space-y-6 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        ...
        {autoRefresh && (
          <div className="text-xs text-slate-500">Auto-refresh: {refreshInterval / 1000}s</div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-slate-900/60 border border-slate-700/70 rounded-2xl p-4 space-y-3">
          ...
          <div className="space-y-2 text-sm text-slate-200">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono text-yellow-400">{stats.recall_requests_last_24h}</span>
            </div>
            <div className="flex justify-between">
        ...
        {/* Column 2: Justice & CP */}
        <div className="bg-slate-900/60 border border-slate-700/70 rounded-2xl p-4 space-y-3">
          <div className="text-xs font-semibold uppercase text-slate-400">Justice & CP</div>
          <div className="space-y-2 text-sm text-slate-200">
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-mono text-orange-400">{stats.violations_last_24h}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Son 7 Gün:</span>
              <span className="font-mono text-red-400">{stats.violations_last_7d}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">Ortalama CP:</span>
              <span className="font-mono text-sky-400">{stats.average_cp.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">LOCKDOWN Kullanıcı:</span>
              <span className="font-mono text-red-600">{stats.lockdown_users_count}</span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-900/60 border border-slate-700/70 rounded-2xl p-4 space-y-3">
          <div className="text-xs font-semibold uppercase text-slate-400">Rejim Dağılımı</div>
      ...
      <div className="bg-slate-900/60 border border-slate-700/70 rounded-2xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-4 gap-4">
          {Object.entries(stats.violation_breakdown).map(([category, count]) => {
            const percentage = violationTotal > 0 ? (count / violationTotal) * 100 : 0
            return (
