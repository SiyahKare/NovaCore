  ...
  return (
    <nav className="flex items-center justify-between py-4 mb-6 border-b border-white/10">
      ...
      <div className="flex gap-6 text-sm items-center">
        {NAV_ITEMS.map((item) => {
          const isActive =
            pathname === item.href || (item.href === '/academy' && pathname?.startsWith('/academy'))
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`transition-colors ${
                isActive ? 'text-aurora-purple font-medium' : 'text-gray-300 hover:text-white'
              }`}
            >
              {item.label}
            </Link>
